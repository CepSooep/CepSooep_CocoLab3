...
int main(int argc, char* argv[]) {
	...
	printf("\timage.colour.mif - 8-colour channel, n x m x 9\n");
	...
	if (f) {
		...
		if (info.biBitCount == 24) {
			...
			faprint(fcol, fm, "Data_radix=UNS;\r\n");
			...
			for(y=height-1; y >=0; y--) {
				...
				for(x=0; x < width; x++) {
					...
					b = (b/32);
					c = (c >= 128 ? 1 : 0);
					buffer = (x<<24|y<<17|r<<14|g<<11|b<<8);

					fprintf(fcol, " %u", buffer);
					fprintf(fm, " %i", c);
				}
				faprint(fcol, fm, ";\r\n");
				if ((x*3) % 4 != 0)
